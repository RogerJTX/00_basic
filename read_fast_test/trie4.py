...
"""
Description:大变双向字典树
迭代次数默认最大999，可以增加但是没必要。其实能深到999层，但效果不好，那样就选其他方式处理。
用于匹配company name，增加效率，找到prefix和suffix，进行优化。
...
if __name__ == "__main__":
    ...
    def judge_children_method(dict_children):
        for key, value in dict_children.items():
            # print('key:', key, ' value:', value)
            print('key:', key)
            judge_children = value.children
            if judge_children:
                print('yes')
                list_each_node.append(judge_children)
                judge_children_method(judge_children)
            else:
                print('finish')
                return None
    ...
    trie_chiledren = trie.root.children
    judge_children_method(trie_chiledren)
